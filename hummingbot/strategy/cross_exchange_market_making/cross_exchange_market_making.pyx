...
cdef class CrossExchangeMarketMakingStrategy(StrategyBase):
    ...
    cdef c_balance_fix_check(self, market_pair):
        """
        Checks whether it has been enough time till rebalance should happen. Returns true if yes.
        """
        cdef:
    ...
    cdef c_fix_balance(self, market_pair):
        """
        Function that is called to fix balance. 
        ...
        #TODO: Figure out how to send orders to maker market without changing c_place_order. Or if necessary, minimal change to c_place_order.
        ...
        if self._keep_target_balance:
            ...
            if self._counter > self._waiting_time and order_size_base > self.min_order_amount:
                ...
                self.log_with_clock(
                    logging.INFO,
                    f"Canceled all maker and taker orders to fix base balance")
                self._fix_counter += 1
                self.log_with_clock(
                    logging.INFO,
                    f"fix_counter is now at: {self._fix_counter} balance fixes since strategy started."
                    f"Timer passed {self._waiting_time} c_ticks, current value of ticks: {self._counter} Order_size base: "
                    f"{order_size_base} Base Balance: {total_base_balance}, Target Balance: {self._target_base_balance}, Diff: {pref_base_min_actual}")

                #If we need to buy to fix target balance: ie target base balance > our current amount
                if pref_base_min_actual > 0:
                    # available balance with a buy order on maker side
                    if (taker_available_balance_quote/mid_price_taker_buy_price) > self._min_order_amount:
                        self.c_place_order(market_pair, True, market_pair.taker, False, self._min_order_amount, Decimal(mid_price_taker_buy_price))
                        self.logger().info(f"Placed rebalancing BUY order of {self._min_order_amount}{market_pair.taker.base_asset} at {market_pair.taker.market}")
                    elif (maker_available_balance_quote/mid_price_maker_buy_price) > self._min_order_amount:
                        self.c_place_order(market_pair, True, market_pair.maker, False, self._min_order_amount, Decimal(mid_price_maker_buy_price))
                ...
                if pref_base_min_actual < 0:
                    ...
                    if taker_available_balance_base > self._min_order_amount:
                        self.c_place_order(market_pair, False, market_pair.taker, False, self._min_order_amount, Decimal(mid_price_taker_sell_price))
                        self.logger().info(f"Placed rebalancing SELL order of {self._min_order_amount}{market_pair.taker.base_asset} at {market_pair.taker.market}")
                    elif maker_available_balance_base > self._min_order_amount:
                        self.c_place_order(market_pair, False, market_pair.maker, False, self._min_order_amount, Decimal(mid_price_maker_sell_price))
                ...
                return True
                #return true, so if there was a mistake, ship the normal proces until it does not return true
        return False
    ...
    cdef c_process_market_pair(self, object market_pair, list active_orders):
        ...
        if self._keep_target_balance and self.c_balance_fix_check(market_pair):
            self.c_fix_balance(market_pair)
            return
